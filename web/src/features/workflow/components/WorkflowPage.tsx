...
const WorkflowPage: React.FC = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setWorkflows(await workflowApi.getWorkflows()); } catch (e) { console.error(e); }
    setLoading(false);
  }, []);

  useEffect(() => { load(); }, [load]);

  const handleToggle = async (wf: Workflow) => { await workflowApi.updateWorkflow(wf.id, { enabled: !wf.enabled }); load(); };
  const handleDelete = async (wf: Workflow) => { if (!confirm(`确定删除 "${wf.name}"？`)) return; await workflowApi.deleteWorkflow(wf.id); load(); };
