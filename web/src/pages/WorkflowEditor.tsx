    ...
    (async () => {
      try {
        ...
        if (json.success && json.data) {
          setWorkflowName(json.data.name);
          setWorkflowMode(json.data.mode || 'independent');
          setTriggerDesc(json.data.triggerDescription || '');
    ...
    try {
      ...
      if (workflowId === 'new') {
        await fetch('/api/workflows', {
          ...
          body: JSON.stringify({ name: workflowName || '新工作流', triggerDescription: finalTriggerDesc, graph, mode: workflowMode }),
        });
      } else {
        await fetch(`/api/workflows/${workflowId}`, {
          method: 'PUT', credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: workflowName, triggerDescription: finalTriggerDesc, graph, mode: workflowMode }),
        });
      }
      onBack();
    } catch (e) { console.error(e); }
