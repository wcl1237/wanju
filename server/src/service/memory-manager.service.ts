...
import { AIConfig, AIMessage, Message } from '../interface';
...
export class MemoryManagerService {
  ...
  async addMessage(
    ...
    // 2. Redis 追加 (只缓存 user / assistant 角色)
  ...
  /**
   * 获取 AI 对话上下文 = 摘要 + Redis 短期记忆 + mem0 长期记忆
   */
  async getAIContext(conversationId: string, userId?: string): Promise<{
    ...
    return {
      messages,
      meta: { hasSummary, shortTermCount, longTermCount, profileCount },
    };
  }

  /**
   * 检查并摘要: 当 token 数接近上下文 80% 时，触发 LLM 摘要压缩
   */
  async checkAndSummarize(conversationId: string): Promise<boolean> {
    ...
    if (estimatedTokens < CONTEXT_TOKEN_LIMIT * SUMMARY_THRESHOLD) {
      return false;
    }
    ...
    try {
      const response = await fetch(`${this.aiConfig.apiBase}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `请将以下对话历史压缩为一段简洁的摘要（约200-300字），保留关键信息、用户需求、已解决的问题和待处理事项。不要用对话格式，用陈述句总结。\n\n${conversationText}`,
          }],
          temperature: 0.1,
        }),
      });
      ...
      summaryContent = summaryContent.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
