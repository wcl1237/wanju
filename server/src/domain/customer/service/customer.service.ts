...
import { CustomerProfile, SaveCustomerInfoDTO } from '../model/customer.model';
...
export class CustomerService {
  ...
  async saveOrUpdate(dto: SaveCustomerInfoDTO): Promise<CustomerProfile> {
    ...
    if (existing) {
      ...
      const entity = this.profileRepo.create({
        id, conversationId: dto.conversationId || null,
        name: dto.name || null, phone: dto.phone || null, email: dto.email || null,
        company: dto.company || null, position: dto.position || null,
        requirement: dto.requirement || null, extra: dto.extra || null,
  ...
  private toProfile(e: CustomerProfileEntity): CustomerProfile {
    return {
      id: e.id, conversationId: e.conversationId || undefined,
      name: e.name || undefined, phone: e.phone || undefined,
      email: e.email || undefined, company: e.company || undefined,
      position: e.position || undefined, requirement: e.requirement || undefined,
      extra: e.extra || undefined, status: e.status as any,
