...
import { IVectorStore } from '../port/vector-store.port';
import { ChunkingService } from './chunking.service';
import { RAGService } from './rag.service';
import { KnowledgeDoc, SearchResult, KnowledgeConfig, RAGContext } from '../model/knowledge.model';
...
export class KnowledgeService {
  ...
  async importDocument(name: string, content: string): Promise<KnowledgeDoc> {
    const docId = uuidv4();
    const chunkMetas = this.chunkingService.smartChunk(
      name, content,
