...
import { Skill, CreateSkillDTO, UpdateSkillDTO } from '../model/skill.model';
...
export class SkillService {
  ...
  async matchByText(text: string): Promise<Skill[]> {
    const allEnabled = await this.skillRepo.find({ where: { enabled: 1 } });
    const lowerText = text.toLowerCase();
    return allEnabled
      .filter(entity => {
        const kws = entity.keywords.split(',').map(k => k.trim().toLowerCase()).filter(k => k.length > 0);
  ...
  private toSkill(e: SkillEntity): Skill {
    return {
      ...
      keywords: e.keywords ? e.keywords.split(',').map(k => k.trim()).filter(k => k.length > 0) : [],
      prompt: e.prompt, icon: e.icon || '⚡', enabled: e.enabled === 1,
